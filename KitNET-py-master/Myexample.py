...
'''
X = pd.read_csv("node43.csv",header=None) #an m-by-n dataset with m observations
X = X[0:20000].copy()
# KitNET params:
...
K = kit.KitNET(X.shape[1],max_autoencoder_size=10,FM_grace_period=None,
                    AD_grace_period=5000,learning_rate=0.1,hidden_ratio=2/3, feature_map = None)
...
x = [i for i in range(len(RMSEs))]
plt.plot(x,RMSEs)
plt.show()

# Here we demonstrate how one can fit the RMSE scores to a log-normal distribution (useful for finding/setting a cutoff threshold \phi)
from scipy.stats import norm
benignSample = np.log(RMSEs[ADgrace+1:X.shape[0]])
