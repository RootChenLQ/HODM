...
def run(exp):
    ...
    Output_DF.to_csv(exp_str+'HGDB_E.csv',mode='a')
    #读取参数文件
    Parent_PATH = ".."
    ...
    datasize = 6000   # 2000 train 6000test
    # MN
    #bufferSize = modelParams['SNParams']['buffer_size']  #4.23 缓存大小
    bufferSize = 3000 #预先训练数据量
    ...
    while times < repeat_time: #重复实验数
        for typeName in anomalyType: #异常类型  #anomaly type normal outlier constant noise
            for subtype in anomalyType[typeName]: #0：T 1：H 2：V
                ...
                typeName = 'constant'
                type = [0]
                data1,outlier_pos =  insert_anomaly(data1, bufferSize, anomaly_num, typeName, type)
                bufferQue = Queue.MNQueue(bufferSize)
                #test_data.to_csv("HGDB_E withnoise.csv") # 存储插入异常的数据
                for i in range(bufferSize):
                    series = data1.iloc[[i]]
                    bufferQue.enqueue(series)  
               
                Global_info , NP , K = update_(bufferQue.df, H_range, sampleforK)
                #queue.clear_count()
                #update_times +=1
                ...
                for row in range(bufferSize,data1.shape[0]):
                    ...
                    is_normal = online_detetion(NP,K,Global_info,series)    
                    #print(row)
                    if is_normal:
                    ...
                    if bufferQue.is_trigger_update():
                        Global_info , NP , K = update_(bufferQue.df, H_range, sampleforK)
                        bufferQue.clear_count()
                        update_times +=1
                ...
                Output_DF.to_csv(exp_str+'HGDB_E.csv',header=0,mode='a')
                #one test
        times+=1
