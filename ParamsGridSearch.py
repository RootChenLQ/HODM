...
_BEST_PARAMS = "bestParams.yaml"


def run(modelParams,bestAcc):
    times = 0
    ...
    while times < 1:
        ...
        sink = Node.CH_Node(0,q,modelParams['CommonParams']['attributes'],modelParams['SNParams']['buffer_size'])
        ...
        #加载数据
        mn1 = Node.MN_Node(8,bufferSize,sqrt_thres,store_pro,sampleforK) #sqrt_thres = 0.5,save_prop=0.8,sampleforK = 50
        mn2 = Node.MN_Node(9,bufferSize,sqrt_thres,store_pro,sampleforK)
        mn3 = Node.MN_Node(10,bufferSize,sqrt_thres,store_pro,sampleforK)
        #加载数据
        #data_len = len(data1)
        ...
        #fp
        #tp
        #kappa1 = Fun.getKappaScore(tp,tn,fp,fn)
        ...
        #calculate Kappa
        kappa2 = Fun.getKappaScore(tp,tn,fp,fn)
        #acc,fpr,tpr,p = online_func_paper.compute_same_num_no_error(detect_all,data_size) #2无异常
        s1 = pd.Series([mn2.ID, tn,fn,fp,tp,acc,fpr,tpr,p,mn2.Recv_pkt_num,mn2.Send_pkt_num,mn2.updateTimes,
                        ...
                        mn2.detect_cells/(mn2.detect_times*27),mn2.Max_Storage,kappa2], #8.27
                       index= save_type)
        df = df.append(s1,ignore_index = True,sort=False)
        #
        #10号节点
        ...
        #calculate Kappa
        kappa3 = Fun.getKappaScore(tp,tn,fp,fn)
        ...
        s1 = pd.Series([mn3.ID, tn,fn,fp,tp,acc,fpr,tpr,p,mn3.Recv_pkt_num,mn3.Send_pkt_num,mn3.updateTimes,
                        ...
                        mn3.detect_cells/(mn3.detect_times*27),mn3.Max_Storage,kappa3],
                       index= save_type)
        df = df.append(s1,ignore_index = True,sort=False)
        TN: 0
        FN: 0
        ...
        if acc > bestAcc:
            with open("out.yaml","w") as outoperation:
                modelParams['Performance']['TN']= tn
                modelParams['Performance']['FN']= fn
                modelParams['Performance']['TP']= tp
                modelParams['Performance']['FP']= fp
                modelParams['Performance']['ACC']= acc
                modelParams['Performance']['FPR']= fpr
                modelParams['Performance']['TPR']= tpr
                modelParams['Performance']['P']= p
                #modelParams['Performance']['Kappa']= kappa1
...
if __name__ == "__main__":
    ...
    with open(_PARAMS_PATH, "r") as f:
        modelParams = yaml.safe_load(f)
    #grif serach
    bastParams = modelParams
    bestKappaScore = 0
    bestAcc = 0
    '''
    buffer_size: 1000 #300 400 500 2000
    store_pro: 0.8    # 0.80, 0.81,...,0.99
    sample_size: 100  #
    sqrt_thres: 0.5   #0.51  0.52 ...1 
    '''
    
    for buffersize_ in range(1,11):
        modelParams['SNParams']['buffer_size'] = buffersize_*100
        for store_pro_ in range(50,100):
            modelParams['SNParams']['store_pro'] = store_pro_*0.1
            for sample_size_ in range(2,5):
               modelParams['SNParams']['sample_size_rate'] = sample_size_*0.1
               for sqrt_thres_ in range(2,10):
                    modelParams['SNParams']['sqrt_thres'] = sqrt_thres_*0.2
                    bestAcc = run(modelParams,bestAcc)
