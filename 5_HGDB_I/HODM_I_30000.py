    ...
    while times < repeat_time:
        ...
        for typeName in anomalyType:
            ...
            for subtype in anomalyType[typeName]:
                ...
                timestamp = time.time()
                #拷贝数据集,避免替代原来的数据集
                begin_ = 10000 - bufferSize  #else model use 10000 to train model
                end_ =  datasize
                ...
                anomaly_num = (int)((datasize - 10000)*anomalyRate)
                ...
                for i in range(member_size):
                    #print(error_l[i])
                    tn,fn,fp,tp, acc,fpr,tpr,p,f1 = Fun.compute_performent(label_l[i],error_l[i],datasize-begin_-bufferSize)
